...
class NJTreeNode:
    ...
    key: Any = None
    contents: Union[str, "NJTreeNode"] = None

    def get_corresponding_items(self, *sources):
        """Get from the inputs the item that this tree node describes"""
...
def find_duals(a):
    ...
    if a._numeric_jacobian_support is not True:
        ...
        members_to_consider = inspect.getmembers(a)
...
def iterate_nj_tree(
    tree: Union[str, NJTreeNode],
    source: Any = None,
    sources: tuple[Any] = None,
):
    """Iterate over tree describing numerical Jacobian inputs/outputs

    Iterates over thee tree generated by find_duals, and uses that tree to walk through
    as many objects as provided in sources in parallel (those objects are assumed to
    match the structure described in the tree).

    Arguments
    ---------

    tree : string or NJTreeNode
        Output of find_duals, or part of that output possibly.

    source : object
        Object from which duals are to be extracted

    sources : sequence of objects
        Provided if multiple objects are to have duals extracted

    """
    if source is not None and sources is not None:
        raise ValueError("Cannot supply both source and sources")
    if source is not None:
        # Do the single input case
        if tree == "dlarray":
            yield source
        else:
            for branch in tree:
                # Note the tuple return value
                (element,) = branch.get_corresponding_items(source)
                yield from iterate_nj_tree(branch.contents, source=element)
    else:
        if tree == "dlarray":
            yield sources
        else:
            for branch in tree:
                elements = branch.get_corresponding_items(*sources)
                yield from iterate_nj_tree(branch.contents, sources=elements)
...
def compute_numeric_jacobians(
    ...
    **kwargs - dict
    ...
    for arg in all_args:
        ...
        if dual_tree is None:
            ...
            if not any_seed:
                ...
                for dual_arg in iterate_nj_tree(dual_tree, arg_deseeded):
    ...
    result_tree = find_duals(result_a)
    ...
    for output_n, output_0 in iterate_nj_tree(
        result_tree, sources=(result_n, result_0)
    ):
